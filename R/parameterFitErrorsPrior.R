parameterFitErrorsPrior <- function(noiseLevel=.01,p=c(1e11,300,300,0,0,.3),pPr=c(1e12,1e4,1e4,0,1e4,0),pm0=c(30.5,16),fradar=233e6,zeroLag=FALSE,nLag=60,llag=233e6/fradar*1e-4,freq=seq(-100000,100000,by=100)*fradar/933.5e6,dp=1e-5){ 
#
...
#  pPr         prior standard deviations of the parameters, 0 for no fit
...
#    lpvar <- noiseLevel**2*Ntau
    ...
    # Fisher information 
    Q <- t(A)%*%solve(Sigma)%*%A + diag(1/(pPr[ind]/pscale[ind])^2)
